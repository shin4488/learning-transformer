...
小数の桁数の比喩がぴったりです。円周率を 3.14159265(32bit)ではなく 3.14(8bit)や 3(4bit)で覚えるようなものです。設計図の寸法なら3.14で十分な場面は多い — LLMのパラメータも同様で、**多少粗くても答えはほとんど変わらない**ことが実験的に分かっています。モデルの出力は数億個のパラメータの掛け算・足し算を合計した結果として決まるので、1個1個の細かい端数のずれは、大量の足し合わせの中に埋もれてしまうのです。

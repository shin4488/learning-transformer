...
第13章で「推論コストがビジネスの生死を分ける」こと、第14章で「生成はループであり、コンテキスト長には $n^2$ の壁がある」ことを見ました。本章はその答え合わせです。巨大なTransformerを現実のサービスとして動かすための代表的な技術を、**「何が問題か → どう解くか → イメージ」** の順で、ひとつずつ見ていきます。実装の細部には立ち入りません。「どういう困りごとに対する、どういう発想の解決策か」を持ち帰ってください。
...
おかしいと思いませんか。ステップ2で計算する「猫」「は」「魚」「が」の処理は、**ステップ1でやったのとまったく同じ**です。第8章の因果マスクを思い出してください — 各トークンは**自分より前しか見ない**ので、「猫」の計算結果は後ろに「好き」が付こうが付くまいが変わりません。つまり生成ループは、素朴にやると**過去の計算を毎回すべてやり直している**のです。100トークン目を作る頃には、1トークン目を100回計算し直していることになります。
...
2. $`\mathbf{q}_5`$ と**ここまでの全トークン(自分を含む)のキー** $`\mathbf{k}_1, \dots, \mathbf{k}_5`$ の内積でスコアを出す
3. softmaxで重みにし、**ここまでの全トークンのバリュー** $`\mathbf{v}_1, \dots, \mathbf{v}_5`$ を重み付き平均する

つまり過去のトークンについて必要なのは**キーとバリューだけ**です。そして因果マスクのおかげで、過去のトークンの $\mathbf{k}, \mathbf{v}$ は**生成が終わるまで二度と変わりません**。ならば答えは一つ — **一度計算したK/Vを保存(キャッシュ)しておき、次のステップでは使い回せばいい**。これが **KVキャッシュ(KV cache)** です。
...
イメージとしては、KVキャッシュは**会議の議事録**です。毎回会議の冒頭から全員の発言をやり直す(キャッシュなし)代わりに、議事録(K/V)を残しておき、新しい発言者は議事録を読んで応答する(キャッシュあり)。
...
第14章の布石を正面から受け取ります。self-attentionはスコア行列 $QK^\top$ 、すなわち**全トークンペアの内積**を計算します(第8章)。トークン数 $n$ に対しペア数は $n^2$ 。この「入力が2倍になるとコストが4倍になる」性質を、**計算量のオーダー記法**で $O(n^2)$(オーダー・エヌ2乗)と書きます。初めて見る記号かもしれませんが、意味は「 $n$ が大きいとき、コストがだいたい $n^2$ に比例して増える」というだけです。
...
「本1冊をまるごと読ませたい」という自然な要望が、2乗の壁のせいで法外なコストになるのが分かります。しかもメモリも同じで、 $n \times n$ のスコア行列を素朴に持てば、10万トークンで $10^{10}$ 個の数値 — これだけでGPUメモリを使い切ってしまいます。
...
$O(n^2)$ への攻め口は複数あります。本章では代表として、**計算のやり方を変える**FlashAttention(15.3節)、**キャッシュを軽くする**MQA/GQA(15.4節)、**長文でも位置情報が壊れないようにする**RoPE/ALiBi(15.5節)を見ます。このほか「全ペアではなく一部のペアだけ見る(疎なattention)」という方向もありますが、本書では名前の紹介に留めます。
...
第9章で、attentionは語順を知らないため**位置エンコーディング**を足すと学びました(sin/cos版や学習型)。しかし従来方式には弱点があります。「位置3000番」を訓練中に一度も見ていないモデルは、3000番の位置信号をうまく扱えません。**コンテキスト長を後から伸ばしたい**(第14章)のに、位置の表現がそれを妨げるのです。また、考えてみれば「絶対的な番地」より「**2つのトークンがどれだけ離れているか(相対位置)**」の方が言語には本質的です。「猫は」と「好き」の関係は、それが文書の10番目にあろうが1万番目にあろうが同じはずですから。
...
- ここで第2章の内積の幾何を思い出してください。**内積は「向きの揃い具合 × 長さ」** で決まるのでした。回転はベクトルの長さを変えないので、回転で変わるのは2つのベクトルの**なす角**だけです。位置 $m$ のクエリを $m \times 10$ 度、位置 $n$ のキーを $n \times 10$ 度回すと、なす角の変化は $(m - n) \times 10$ 度 — **差だけ**に依存します
...
![RoPEのイメージ: 回しても間の角度は変わらない](images/ch15-rope-clock.png)

(図: 青い針がクエリ、オレンジの針がキー。どちらのペアも「2トークン差」なので、それぞれの位置ぶん回転させても、針の間のなす角(緑)は同じ20°のまま。内積はこのなす角で決まるため、attentionには相対位置だけが効く)
...
**読み下し**: トークン $i$ から $j$ へのattentionスコアは、通常の内積から「傾き $m$ × 2つのトークンの間の距離」を引いたもの。遠いトークンほど減点され、softmax後の重みが小さくなる。
...
**量子化(quantization)** は、パラメータ1個1個の数値を、**より少ないビット数で大まかに記録し直す**(細かい端数を捨てて、あらかじめ決めた粗い目盛のどれかに丸める)ことでメモリを削ります。
...
小数の桁数の比喩がぴったりです。円周率を 3.14159265(32bit)ではなく 3.14(8bit)や 3(4bit)で覚えるようなものです。設計図の寸法なら3.14で十分な場面は多い — LLMのパラメータも同様で、**多少粗くても答えはほとんど変わらない**ことが実験的に分かっています。モデルの出力は膨大な数のパラメータの掛け算・足し算を合計した結果として決まるので、1個1個の細かい端数のずれは、大量の足し合わせの中に埋もれてしまうのです。
...
1個1個には最大0.06程度の誤差が入りますが、誤差の向きはバラバラなので、第2章で学んだ内積(掛けて足す)を取ると**プラスとマイナスの誤差が打ち消し合い**、層の出力はほとんど変わりません。これが「桁を削っても、大量の足し合わせの結果はほとんど変わらない」ことの数値的な正体です。
...
普通の訓練(第10章)は上の行 — 「正解は『好き』、他は全部ダメ」しか教えません。蒸留は下の行を教えます。そこには「**『大好き』もかなりあり得る。『苦手』も文法的には成立する**」という、先生が長い訓練で獲得した**単語同士の関係の知識**が丸ごと写っています。生徒は「先生の分布に自分の分布を近づける」ように訓練され(距離の測り方には第5章の交差エントロピーとよく似た式を使います)、正解ラベルだけから学ぶより効率よく賢くなります。

### 15.7.3 イメージ

名人の将棋を学ぶとき、「最善手はこれ」という結果だけの棋譜より、「**この手が7割、あの手も2割はあった**」という迷い方まで聞ける弟子の方が早く強くなる — 蒸留はそういう技術です。実務では、大きいモデルの賢さをスマホなどの小型機器でも使えるようにする標準手段になっています。
...
**読み下し**: 学習するパラメータは元の約0.39%、つまり**0.4%未満**。それでも多くの微調整タスクで、全パラメータ更新に匹敵する品質が出る。

なお、学習開始時に元モデルの知識を壊さない工夫もスマートです。通常、行列 $A$ は小さな乱数で初期化し、行列 $B$ は**すべて0**で初期化します。すると学習開始直後は $BA = 0$ となり、 $W' = W + 0 = W$ で「元モデルと完全に同一の状態」から安全に微調整をスタートできるのです。

### 15.8.3 イメージとうれしい性質

要するにLoRAは、「**本体である $W$ には一切手を加えず、微調整で変えたい差分だけを小さな行列 $B, A$ に学習させて、使うときに足し合わせる**」方法です。微調整で変えたいことは「口調を丁寧に」「社内用語を覚える」など、事前学習で得た知識の総量に比べればごくわずかな変化なので、小さな行列に収まる程度の差分で足りるのです。
...
量子化した本体にLoRAを組み合わせる手法(QLoRA)により、「家庭用GPU1枚でLLMを微調整する」ことすら現実になりました。第2章の「行列の形と積の定義」という地味な知識が、「巨大モデルを個人でも扱えるようにする」という変化を支えている — 本書でも特に印象的な回収です。
...
イメージとしては**大病院**です。病院全体には何百人もの専門医がいます(巨大な知識)が、あなたが受診するのは症状に応じた2人だけ(小さい計算)。全員があなたを診る必要はないのです。

ただし欠点もあります。計算に使うのは選ばれた2人だけでも、トークンごとにどの専門家が選ばれてもいいように、**全員ぶんのパラメータをあらかじめGPUメモリに載せておく**必要があります。つまり、計算量は節約できても、メモリ使用量は節約できません。また、何もしないとルーターが特定の専門家ばかり選ぶようになり、選ばれない専門家がまったく育たなくなるため、訓練時には「どの専門家にも仕事がまんべんなく回るようにする」仕掛けを追加する必要があります。

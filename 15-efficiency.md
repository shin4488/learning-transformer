...
第13章で「推論コストがビジネスの生死を分ける」こと、第14章で「生成はループであり、コンテキスト長には $n^2$ の壁がある」ことを見ました。本章はその答え合わせです。巨大なTransformerを現実のサービスとして動かすための代表的な技術を、**「何が問題か → どう解くか → 直感」** の3拍子で、ひとつずつ見ていきます。実装の細部には立ち入りません。「どういう困りごとに対する、どういう発想の解決策か」を持ち帰ってください。
...
図解します(本章の最重要図なので、規約どおりASCIIとMermaidの両方で描きます)。

```text
【KVキャッシュなし】ステップ4(「好き」の次を作る)で起こること

  猫   は   魚   が   好き
  ↓    ↓    ↓    ↓    ↓
 QKV  QKV  QKV  QKV  QKV     ← 5トークン全部でQ/K/Vを計算し直し
  └────┴────┴────┴────┘
       全ペアのattention           ← 計算量はトークン数の2乗

【KVキャッシュあり】同じステップ

  猫   は   魚   が    好き
  💾   💾   💾   💾    ↓
 (保存済みのK/V)      q₅k₅v₅    ← 新トークン1個だけ計算
  k₁v₁ k₂v₂ k₃v₃ k₄v₄  │
   └────┴────┴────┴────┘
    q₅ × 保存済みK の内積 → softmax → 保存済みV の重み付き平均
    (k₅, v₅ をキャッシュ庫に追記して次のステップへ)
```
...
graph TD
    ...
    style CACHE fill:#bbdefb,stroke:#1565c0,stroke-width:2px
```
...
```text
  ...
  FFNや埋め込みの計算はほぼ「比例」で伸びるのに対し、
  attentionのスコア計算だけが「2乗」で伸びる。
  文脈が長くなるほど、attentionが全コストの主犯になっていく。
```
...
**RoPE(Rotary Position Embedding、回転位置埋め込み)** は、現在もっとも広く使われる方式です。発想はエレガントです。
...
```text
【RoPEの直感: 回しても「間の角度」は変わらない】

   位置1と位置3のペア              位置5と位置7のペア
   (それぞれ10度・30度回転)        (それぞれ50度・70度回転)
...
両者に共通する哲学は「**絶対番地をやめ、相対関係を直接扱う**」です。住所(絶対位置)ではなく「隣の家」「3軒先」(相対位置)で考えれば、どんなに長い通りでも同じルールが使えます。
...
```text
  ...
  ┌───────────────┐      ┌──┐   ┌──────────┐   ┌───────────────┐
  │               │      │  │   │──────────│   │               │
  │  4096 × 4096  │  +   │  │ × │  8 ×4096 │ = │  4096 × 4096  │
  │   1678万個     │      │  │   └──────────┘   │ (形は同じだが │
  │  (動かさない)  │      │  │    横長で薄い     │  中身は単純)  │
  └───────────────┘      └──┘                  └───────────────┘
                        4096×8
                        縦長で薄い
        学習するのは B と A の中身だけ(合計 6.6万個)
```
...
量子化した本体にLoRAを組み合わせる手法(QLoRA)により、「家庭用GPU1枚でLLMを微調整する」ことすら現実になりました。第2章の「行列の形と積の定義」という地味な知識が、巨大モデルの民主化を支えている — 本書でも指折りの痛快な回収です。
...
```text
        ...
        ┌────────┬──────┼──────┬────────┐
        v        v      v      v        v
      FFN1     FFN2   FFN3   FFN5     FFN8
     (休み)   ★稼働  (休み)  ★稼働   (休み)   … 専門家8人のうち2人だけ働く
                │             │
                └──重み0.6────┴──重み0.4──> 合流 → このトークンの出力
```
...
これで、本書で予定していた技術の旅はすべて終わりです。数の並べ方(第2章)から始まり、attentionの発明(第8章)、訓練(第10章)、対話AI化(第12章)、規模の法則(第13章)、生成(第14章)、そして現実に動かす工夫(本章)まで — ばらばらに見えた部品が、いまやあなたの中で1本につながっているはずです。[第16章 まとめと次の一歩](16-conclusion-and-next-steps.md)では、全16章を1枚の地図として振り返り、よくある質問に答え、この先の学習ロードマップと用語集をお渡しします。

...
- **貪欲法(greedy decoding)** と、その「単調・繰り返し」という弱点
...
これだけです。**文章は1文字も出てきません**。「じゃあ、どうやって文章に?」— 答えはごく単純で、「**選んだトークンを入力の末尾にくっつけて、もう一度モデルに入れる**」を繰り返すのです。
...
1. **モデル自身はまったく変わらない**。ループの間、パラメータ $\theta$ は固定です。文章を「書いている」ように見えるのは、外側のループがトークンを積み上げているからです
...
4ステップ目に出てきた **EOS(End of Sequence)** は「文章はここでおしまい」を意味する特別なトークンです(14.8節で詳述)。これが選ばれた瞬間、ループは停止し、完成品「猫は魚が好きだ。」が返されます。
...
なぜこうなるのか。「猫は魚が好きだ。」まで出た時点で、文脈に「猫は魚が」というパターンが強く存在します。モデルは訓練で「直前の文脈と似た表現は繰り返されやすい」ことも学んでいるため、同じフレーズの確率が少し上がる → 選ばれる → 文脈にそのフレーズが2回現れ、さらに確率が上がる → …という**自己強化の悪循環**に落ちるのです。貪欲法は毎回必ず確率最大を選ぶため、一度この循環に入ると絶対に抜け出せません。
...
なぜこうなるのかの直感も整理しておきましょう。
- **$T \to 0$ で貪欲法になる理由**: 小さな数で割るとスコアの差が何十倍にも拡大されます。指数関数 $e^x$ は少しの差でも激しく差が開く(第1章)ため、最大スコアの候補だけが圧倒的に巨大になり、他を0%に押し潰します。
- **$T \to \infty$ で均等になる理由**: 巨大な数で割るとすべてのスコアが $s_i/T \approx 0$ に縮んでいきます。 $e^0 = 1$ ですから、すべての候補が同じ「1」になり、全候補がまったく等確率(3候補なら各33.3%)になります。

分布の形をグラフで見比べると一目瞭然です。

```mermaid
xychart-beta
    title "T = 0.1(ほぼ貪欲法): 1位に集中"
    x-axis ["好き", "大好き", "苦手"]
    y-axis "確率" 0 --> 1
    bar [0.99995, 0.00005, 0]
```

```mermaid
xychart-beta
    title "T = 1.0(素のまま): モデルの素顔"
    x-axis ["好き", "大好き", "苦手"]
    y-axis "確率" 0 --> 1
    bar [0.665, 0.245, 0.090]
```

```mermaid
xychart-beta
    title "T = 2.0(平らに): 差が縮まる"
    x-axis ["好き", "大好き", "苦手"]
    y-axis "確率" 0 --> 1
    bar [0.506, 0.307, 0.186]
```
...
重要な注意: **温度はモデルを賢くも愚かにもしません**。スコア(モデルの判断)そのものは少しも変わらず、変わるのは「その判断をどれくらい忠実に/大胆にくじ引きに反映するか」だけです。料理にたとえるなら、素材(スコア)は同じで、味付けの濃さを変えているのです。
...
温度付きサンプリングにはまだ弱点があります。語彙は5万語もあるので、1語1語はごくわずかな確率しかない「明らかに変な単語」も、**何万語ぶんも集まれば無視できない合計確率**になります(これを分布の**長い尾(long tail)** と呼びます)。実際、14.1節の表では、上位5語以外の「残り約5万語」に合計 0.11 もの確率が残っていました。確率どおりにくじ引きをすれば、およそ10トークンに1回はこの尾から引いてしまう計算です。「猫は魚が**炭酸**」のような脱線をされては困ります。そこで「くじ引きの参加資格を上位に限る」という発想が生まれました。
...
### 14.5.3 top-p(nucleus)サンプリング: 累積確率 p まで残す
...
ビームサーチは機械翻訳や要約で長年の定番です。これらは**入力が決まれば出力すべき内容もほぼ決まる**タスクなので、「もっとも確率の高い1本」を探す価値があるからです。ところが雑談や物語のような**自由生成**でビームサーチを使うと、悪いことが2つ起きます。
...
**1. EOSトークン**。第6章で語彙とトークンを学びましたが、語彙には普通の単語のほかに**特別トークン(special tokens)** がいくつか含まれています。その代表が **EOS(End of Sequence)** = 「文章の終わり」を表すトークンです。訓練データの各文書の末尾にEOSを付けておくことで、モデルは「話がまとまったらEOSを出す」ことも**次単語予測の一部として**学習します(第10章の訓練と何も変わらない、というのがよくできているところです)。生成の途中で「次のトークン」としてEOSが選ばれたら、モデルが「文章はここで終わり」と判断したということなので、そこでループを打ち切ります。
...
1. **訓練時に見た長さを超えると性能が落ちる**: 位置の情報(第9章の位置エンコーディング)は訓練で見た範囲でしかうまく調整されていません
...
- モデルは**1トークンを出すたびに、決まった量の計算(第9章で見た、積み重ねたブロック全段ぶん)しかできません**。問題が難しかろうと簡単だろうと、1トークンあたりの「考える時間」は同じです
...
- **貪欲法**(常に確率最大)は単純だが、単調・同じフレーズの繰り返し・局所最適という弱点を持つ

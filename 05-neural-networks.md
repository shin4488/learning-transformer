...
- **逆伝播** — 連鎖律を使って、膨大なパラメータの勾配を一気に計算する仕組み(イメージで理解)
- 深層学習の登場とGPUの役割
...
ポイントは3つあります。**複数の入力信号**、**入力ごとの重要度（つながりの強さ）**、そして**一定の強さを超えたら反応する（しきい値）**という点です。この3つの特徴を数式としてモデル化したものが人工ニューロンです。

実際の脳の生物学的な再現そのものを目指しているわけではありません。飛行機が鳥の翼をヒントにしつつも独自の力学で空を飛ぶように、ニューラルネットワークは神経細胞の着想をベースにしながら、ここまでの章で学んだ数学（内積・関数・微積分）を土台として組み立てられています。
...
活性化関数のないニューロンの計算は「行列を掛けて、ベクトルを足す」だけです(5.5節で見るように、ニューロンを並べた層は行列で書けます)。このような、掛けて足すだけの変換をおおまかに**線形(linear)** な変換と呼びます。一次関数 $wx + b$ の仲間、まっすぐな変換です。
...
現在の深層学習で最も広く使われるのは、**ReLU(Rectified Linear Unit、レルー)** という、非常に単純な関数です。
...
ReLUの長所は、プラス側の傾きが常に1で、シグモイドのような「端で勾配が消える」問題がプラス側では起こらないこと、そして計算がきわめて軽いこと($\max$ を取るだけ)です。この単純さは、何十億回と計算を繰り返す深層学習では大きな利点になります。
...
この三段論法は本書の後半で何度も効いてきます。特に第7章では「行列計算に**できない**部分(逐次処理)を抱えたモデル(RNN)がGPUを活かせずに苦しみ、行列計算だけで構成されたTransformerに道を譲る」という、本書の中心となる話が展開されます。
...
いよいよ、この章の主役である **softmax(ソフトマックス)** の登場です。本書の後半で繰り返し主役を務める関数ですので、ここでしっかり手を動かして身につけましょう。
...
3. **差を強調するため**。第1章で見た「指数関数の爆発的増加」がここで活きます。スコアの差 $2 - 1 = 1$ は、指数を通すと**倍率** $e^2 / e^1 = e \approx 2.7$ 倍の差になります。スコアで3差なら約20倍、5差なら約148倍。優勢な候補に確率をぐっと寄せる、メリハリの効いた変換になるのです。
...
> [!IMPORTANT]
> **予告**: softmaxは第8章で「どの単語にどれだけ注意を向けるか」の重み計算に、第10章で訓練時の確率出力に、第14章では「温度」というつまみ付きで文章生成に登場します。ここでの手計算が、本書後半を読み進めるための土台になります。
...
なぜこの式が損失（ペナルティ）として優れているのでしょうか。確率は必ず 0 から 1 の間の値をとります。

- **確率が $1.0$（完璧な正解）のとき**: $\log(1) = 0$ なので、損失は **$0$（ペナルティなし）** です。
- **確率が $0$ に近づく（大外れの）とき**: 第1章で見たように、1より小さい数の対数は負になり、0に近づくほど急激に奈落へ落ちていきます。ここにマイナスを掛けると、損失は **正の巨大な値（急激に増大）** に跳ね上がります。

つまり、「正解を堂々と当てれば罰金ゼロ、正解を軽視して外すほど罰金が爆発的に跳ね上がる」という採点の仕組みが、 $-\log p$ というたった数文字の式で実現できるのです。
...
1層目のある重み $w$ が損失 $L$ に与える影響を考えます。 $w$ を少し動かすと、1層目の出力 $\mathbf{h}_1$ が少し変わり、それが2層目の出力 $\mathbf{h}_2$ を少し変え、それが出力 $\mathbf{y}$ を変え、最終的に損失 $L$ が変わります。影響は層から層へ順々に伝わって届くのです。連鎖律によれば、この一連の変化率は各段の変化率の掛け算です。
...
```mermaid
flowchart TD
    subgraph 順伝播["順伝播: 予測(上から下)"]
        X["入力 x"] --> H1["1層目の出力"]
        H1 --> H2["2層目の出力"]
        H2 --> Y["予測 y"]
        Y --> L["損失 L"]
    end
    subgraph 逆伝播["逆伝播: 勾配(下から上)"]
...
完全な数式展開はしませんが、ブラックボックスでもありません。あなたはすでに部品(微分・連鎖律・合成関数)をすべて持っていて、それが効率よく組み合わされているだけ — そう安心して先へ進んでください。なお、この「同じ変化率を何段も掛け算する」構造には副作用もあります。掛ける数が小さいと積はどんどん0に近づき(シグモイドの平らな領域を思い出してください)、深いネットワークほど勾配が消えやすいのです。この**勾配消失**の問題は第7章(RNNの限界)と第9章(残差接続)で鍵になります。

## 5.10 深層学習の登場とGPU

最後に、この章の道具が歴史をどう動かしたかを駆け足で見ます。

ニューラルネットワークのアイデア自体は古く、1950年代からあります(単層のパーセプトロン)。逆伝播による多層の学習も1980年代に広まりました。しかし長らく、ニューラルネットワークは「理屈は面白いが、実用では他の手法に勝てない」存在でした。層を深くすると勾配消失などで学習が難しく、そもそも計算力もデータも足りなかったのです。

転機は2010年代です。

- **データ**: インターネットの普及で、画像もテキストも桁違いの量が手に入るようになった
- **計算力**: ゲーム用に進化していたGPUが、5.5節で見たとおり行列計算の塊であるニューラルネットワークと非常にうまく噛み合った
- **技術**: ReLU(勾配消失の緩和)をはじめ、深いネットワークを安定して学習させる工夫が揃ってきた

...
- **パープレキシティ** — 「モデルが平均何択で迷っているか」を表す、モデルの性能の指標
...
なお、このように「入力に正解の並びをそのまま与えて、全位置で次を予測させる」訓練方式を **教師強制(teacher forcing)** と呼びます。位置3の予測が外れても、位置4の入力には(モデルの予測ではなく)正解の「が」を使う、という意味です。つまり訓練中は、途中の予測が当たっても外れても、入力にはすべて正解の並びを使う、ということです。
...
開始直後の損失(約10.8)は当てずっぽうの値です(語彙5万語なら $\ln 50000 \approx 10.8$)。序盤の急降下では「は」「が」などの頻出パターンをまず覚え、その後のゆるやかな長い下りで文法→知識→推論と深い能力が育っていきます。

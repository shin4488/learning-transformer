...
コンピュータに何かをさせる伝統的な方法は、人間が手順(ルール)をすべて書き下すことです。たとえば「消費税込みの価格を計算する」なら、

> 入力された価格に 1.1 を掛けて出力せよ

というルールを人間が考え、機械はそれを忠実に実行します。第1章の言葉で言えば、人間が関数 $f(x) = 1.1x$ を**自分で設計して**機械に渡すわけです。
...
学習の全体は「予測 → 採点 → 傾き計算 → 更新」のループです。この図は本書全体で何度も戻ってくる最重要の図なので、Mermaid と ASCII の両方で示します(第10章でTransformerを訓練するときも、ループの中身が豪華になるだけで骨格はこのままです)。
...
```text
 ...
 ┌─────────────┐   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
 │ ① 予測      │   │ ② 採点      │   │ ③ 傾き      │   │ ④ 更新      │
 │ ŷ = f_θ(x) │──▶│ 損失 L を計算│──▶│ 勾配 ∇L を計算│──▶│ θ ← θ − η∇L │
 └─────────────┘   └─────────────┘   └─────────────┘   └──────┬──────┘
                                                              │
                        損失が十分小さくなるまで ①〜④ を繰り返す
```
...
これが機械学習の「学習」の正体です。魔法でも何でもなく、**「外れ具合を測る → 傾きを計算する → 傾きと逆に少し動かす」をひたすら繰り返す**、それだけです。LLMの訓練も、パラメータが数千億個になり損失関数が複雑になるだけで、やっていることはこの表と同じです。
...
```text
 ...
 L                           L
  ...
  |    ●＼＿                  |    ＼   ／    どんどん高く跳ね上がる
  |      ●＼＿                |     ＼ ／● ステップ1
  |_________＼●＿____→ w      |______＼●／________→ w
       するすると谷底へ              ↑谷底(素通り)
...
```text
 ...
 y                             y        ∩
  ...
  |      傾向をとらえる          |    ぐにゃぐにゃ曲線
  +――――――――――――→ x              +――――――――――――→ x
  初見の×も直線の近くに来そう     初見の×は曲線から外れそう
```

逆に、モデルが単純すぎて訓練データすらろくに合わせられない状態は**未学習(underfitting)** と呼びます。ちょうど良い複雑さのモデルを、ちょうど良い加減まで学習させることが大切です。

この「大きすぎるモデルは過学習する」という常識は、機械学習の基本中の基本です。……なのですが、実は本書の後半(第13章)で、LLMがこの常識を心地よく裏切る話が出てきます。楽しみにしていてください。
...
分類モデルの出力はどんな形が良いでしょうか。「猫です」と1つだけ答えさせる(白黒つける)こともできますが、それより**各選択肢にどれくらい自信があるかを丸ごと出させる**ほうがずっと便利です。
...
```text
 「猫は魚が ___」の次の単語の予測(確率分布のイメージ)

 好き     ████████████████████ 0.40
 嫌い     █████ 0.10
 食べたい ████ 0.08
 見ている ███ 0.06
 …        (残りの単語に 0.36 が薄く分配。合計はきっちり 1)
```

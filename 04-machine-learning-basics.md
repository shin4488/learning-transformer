...
![家賃データと引きたい直線](images/ch04-rent-fit.png)

(図: 青い点が5件のデータ、オレンジの直線がモデル $\hat{y} = wx + b$ の一例)
...
1. **プラスとマイナスの外れが打ち消し合うのを防ぐ。** 1万円高く予測した部屋と1万円安く予測した部屋があると、単純な差の合計は $(+1) + (-1) = 0$ になってしまいます。「どちらも外しているのに合計すると完璧に当たっていることになってしまう」のを防ぐため、2乗してすべて正の数にします。
2. **大きな外れを重く罰することができる。** 2万円の外れは、2乗すると $4$ 。1万円の外れ($`1`$)の4倍のペナルティになります。「大外しは特に避けたい」という要求が自然に式へ反映されます。
3. **滑らかな曲線になり、微分（傾きの計算）ができる。** 符号をプラスにするだけなら絶対値 $| \hat{y}_i - y_i |$ でも良さそうに思えます。しかし絶対値のグラフは底の部分が「V字型」に尖っており、折れ曲がった先端では微分（接線の傾き）が計算できません。一方、2乗した関数はなめらかなお椀型の「谷」になるため、どこでもきれいに微分でき、次節で学ぶ勾配降下法と抜群に相性が良いのです。
...
- 現在地: $w = 1.5$ 。損失 $L(1.5) = \frac{1}{2}\left( (-0.5)^2 + (-1)^2 \right) = \frac{1}{2}(0.25 + 1) = 0.625$
...
```mermaid
xychart-beta
    title "勾配降下法の軌跡: 損失はステップごとに約4分の1へ"
    x-axis "ステップ" [0, 1, 2, 3, 4]
    y-axis "損失 L" 0 --> 10
    line [10, 2.5, 0.625, 0.156, 0.039]
```
...
- ステップ1: $w = 0 - 0.5 \times (-10) = 5$ — 谷底の $2$ を**飛び越えて**反対側の斜面へ
...
- **緑色の水平な直線** = 目指している**正解の値($`w = 2`$ 、損失が最小になる値)**。ずっと一定なので真横にまっすぐ伸びています
- **もう1本の折れ線** = 各ステップでの $w$ の現在地。この線が緑の線に近づけば「学習が進んでいる」、離れれば「発散している」ことを意味します

```mermaid
xychart-beta
    title "(1) ηが適切 0.1: するすると正解の w=2 に近づく"
    x-axis "ステップ" [0, 1, 2, 3, 4]
    y-axis "パラメータ w の位置" 0 --> 2.5
    line [0, 1, 1.5, 1.75, 1.875]
    line [2, 2, 2, 2, 2]
```

```mermaid
xychart-beta
    title "(2) ηが大きすぎ 0.5: 正解の w=2 を毎回通り過ぎ、振れ幅が拡大して発散"
    x-axis "ステップ" [0, 1, 2, 3, 4]
    y-axis "パラメータ w の位置" -10 --> 10
    line [0, 5, -2.5, 8.75, -8.13]
    line [2, 2, 2, 2, 2]
```

```mermaid
xychart-beta
    title "(3) ηが小さすぎ 0.01: 方向は正しいが、正解の w=2 には遠く届かない"
    x-axis "ステップ" [0, 1, 2, 3, 4]
    y-axis "パラメータ w の位置" 0 --> 2.5
    line [0, 0.1, 0.2, 0.29, 0.37]
    line [2, 2, 2, 2, 2]
```
...
損失は $L(0, 0) = \frac{1}{2}(9 + 25) = 17$ から $L(1.3, 0.8) = \frac{1}{2}\left( (2.1 - 3)^2 + (3.4 - 5)^2 \right) = \frac{1}{2}(0.81 + 2.56) \approx 1.7$ へ、1ステップで激減しました。
...
![良い当てはめと過学習の比較](images/ch04-overfitting.png)
...
これはまさに、第3章で学んだ**確率分布**です(すべて0以上で、合計が1)。第3章の最後に「『文の次に来る単語』は確率分布と見なせる」という話をしました — 「猫は魚が」の次は「好き: 50%、食べ: 20%、大好き: 15%、……」。分類モデルの出力を確率分布にするというのは、あの見方をそのまま実装することなのです。
...
xychart-beta
    ...
    x-axis ["好き", "食べ", "大好き", "嫌い"]
    y-axis "確率" 0 --> 0.6
    bar [0.50, 0.20, 0.15, 0.10]
```

図に載せた4語以外の残りの単語には 0.05 が薄く分配されており、全体の合計はきっちり1になります(第3章3.9.1節の表と同じ分布です)。

ただし、モデルの計算(掛けて足す)の生の結果は、マイナスになったり合計が1にならなかったりする「ただの数値の並び」です。これを確率分布の条件(全部0以上・合計1)に整える専用の変換が必要で、それが**softmax(ソフトマックス)** です。softmaxは本書でも特に大事な関数なので、次の第5章でじっくり導入します。ここでは「生の点数を確率分布に変換する関数が要る」とだけ覚えておいてください。
...
- **分類問題**の出力は確率分布にするのが良い。「次の単語の予測」は巨大な分類問題である(生の点数を確率分布にする関数 = softmax は次章)
- 特徴量を人が設計する時代から、**表現そのものを学習する深層学習**の時代へ — その主役がニューラルネットワーク

## 次の章へ

次章では、いよいよ**ニューラルネットワーク**の中身を開けます。1個の人工ニューロンが第2章の「内積」そのものであること、層を重ねることが第1章の「関数の合成」であることを確かめ、そして本書の後半で繰り返し使う **softmax** と**交差エントロピー**を手計算で身につけます。

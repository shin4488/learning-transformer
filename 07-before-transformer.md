...
- ボトルネックを打ち破った**Attention(注意機構)の誕生**——「重み付き平均で情報を集める」という発想
- それでも残ったRNNの宿命的な問題(逐次処理・並列化不可)と、"Attention Is All You Need" への道
...
> 「猫は魚が___」の空欄に何が来るかは、確率分布として考えられる。この見方が本書全体の核心になる。

いよいよこの布石を回収します。この「次に来る単語を確率分布として予測する仕組み」には正式な名前があります。**言語モデル(language model)** です。
...
「次の単語を当てるだけの機械」は、一見地味です。しかしこれが驚くほど万能であることが、本書の残りを通して明らかになっていきます。さしあたり2つだけ挙げます。
...
**どんなに大量の文章を集めても、ほとんどの並びは一度も出現しません**。出現回数0の並びの確率は見積もれない(あるいは0になってしまう)ので、$n$ は実用上3〜5程度が限界でした。第1章で見た「指数の爆発的増加」が、ここでは敵として立ちはだかるわけです。
...
2. $\mathbf{h}_t$ は $\mathbf{h}_{t-1}$ に依存し、$\mathbf{h}_{t-1}$ は $\mathbf{h}_{t-2}$ に依存し……と、**メモは玉突きで過去全体とつながっています**。原理的には、$\mathbf{h}_t$ には1トークン目からの情報が(間接的に)全部流れ込んでいます。n-gramのような「$n-1$ 単語で強制打ち切り」がありません。
...
同じ図をASCIIでも示します(5つの「RNNセル」はすべて同じ重み $W_h, W_x$ を使っています)。

```text
   x1:猫    x2:は    x3:魚    x4:が    x5:好き   ← 各トークンの埋め込み
     │        │        │        │        │
     ▼        ▼        ▼        ▼        ▼
   ┌────┐  ┌────┐  ┌────┐  ┌────┐  ┌────┐
h0→│RNN │→│RNN │→│RNN │→│RNN │→│RNN │→ h5
   │セル│h1│セル│h2│セル│h3│セル│h4│セル│   (文全体のメモ)
   └────┘  └────┘  └────┘  └────┘  └────┘
     │        │        │        │        │
     ▼        ▼        ▼        ▼        ▼
  次単語の  次単語の  次単語の  次単語の  次単語の   ← 各時点でsoftmaxで
  確率分布  確率分布  確率分布  確率分布  確率分布     確率分布を出せる
   (すべてのセルは同一の重み W_h, W_x を使い回す)
```
...
- $W_x = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}$(そのまま通す行列)、$W_h = \begin{pmatrix} 0.5 & 0 \\ 0 & 0.5 \end{pmatrix}$(各成分を半分にする行列)
...
たった5トークンでこれです。もっと長い文なら? ——これが次節のテーマ、RNN最大の弱点です。
...
逆に、もし $W_h$ が情報を2倍にする行列だったら $2^{100} \approx 1.3 \times 10^{30}$——今度は数値が爆発します。**縮めば消滅、伸びれば爆発**。ちょうど1倍をずっと保つ綱渡りは、学習でパラメータが動き続ける以上、まず維持できません。
...
勾配消失の帰結は残酷です。勾配は「損失を減らすためにパラメータを動かすべき方向」(第3・4章)でした。遠い過去に関する勾配が0なら、**モデルは遠い過去との関係を学習できません**。つまり素朴なRNNは、原理的には過去全体を見られる構造なのに、**学習の力学として長距離依存が学べない**のです。「猫は 昨日 隣の 家の 庭で 見つけた 小さな 魚が」まで読んで「好き」の主語を猫と結びつける——n-gramで諦めたあの課題が、RNNでも(素朴なままでは)解けませんでした。
...
勾配消失との長い戦いの末に生まれた改良版が **LSTM(Long Short-Term Memory、長・短期記憶)** です。ここでは概要だけ掴めば十分です(内部の完全な数式は本書では使いません)。
...
ポイントは、ゲートを全開(1に近い値)にすれば**記憶が変換行列を通らずにほぼそのまま流れ続ける**ことです。掛け算の鎖に「1を掛け続けるルート」ができるので、勾配も消えにくくなります。しかも、どのゲートをいつ開閉するかは**すべて学習で決まります**。「主語の情報は文末の動詞まで保持せよ」といった戦略を、データから自動的に身につけるのです。

### 7.5.2 RNN/LSTMの栄光の時代

LSTM(と、その簡略版のGRU)は、2010年代半ばの自然言語処理の主役でした。機械翻訳、音声認識、スマートフォンの音声アシスタント、予測変換——身の回りの言語AIの多くがRNN系で動いていた時代があります。特に機械翻訳での成功が、次節のseq2seqです。
...
```text
 エンコーダ(日本語を読む)                デコーダ(英語を書く)
                                  │
  猫 → は → 魚 → が → 好き          │       Cats → like → fish
  ↓    ↓    ↓    ↓    ↓           │        ↑      ↑      ↑
...
「重み付き平均」——第3章で学んだ**期待値**の計算とまったく同じ形です(確率で重み付けして足す、あの形)。布石がまたひとつ回収されました。
...
各行が「その瞬間の視線の配り方」です。もはや1本の要約に頼らず、**毎ステップ、原文全体を見渡して必要な場所に焦点を合わせる**——「注意」という名前の通りです。
...
「スコアはどうやって計算するのか?」「何を根拠に0.85などの重みが出てくるのか?」——当然の疑問ですが、**ここでは踏み込みません**。スコアの計算方法、Q・K・Vという役割分担、行列でまとめて書く方法など、Attentionの数式の詳細はすべて**次の第8章で徹底的に**扱います。本章で掴んでほしいのはただ1点、

> **Attention = 関連度に応じた重みで、全部の情報の重み付き平均を取る仕組み**

という発想です。ヒントだけ言えば、「関連度の採点」に使われるのは、第2章から繰り返し登場しているあの演算——内積(=類似度)です。

Attentionを組み込んだseq2seqは、長文翻訳の品質を劇的に改善しました。ボトルネックは解消され、2016年頃には大手の翻訳サービスもこの方式(RNN + Attention)に切り替わっています。めでたしめでたし——とはいかなかったのが、次節です。
...
第5章で強調した事実を思い出してください。

> ニューラルネットワークの計算は行列演算で書ける。そしてGPUは、**巨大な行列演算を一括で並列処理する**のが得意だから深層学習が花開いた。
...
これは訓練で致命傷になります。言語モデルを賢くするには膨大な文章を学習させたいのに(この規模の話は第10章・第13章で)、1文書の中を1トークンずつしか進めないのでは、時間がいくらあっても足りません。**モデルとデータを大きくしたくても、逐次性が天井になる**のです。
...
- Attentionは「全部のメモの重み付き平均」であり、計算の実体は行列演算——**全トークンを一括で並列処理できる**(GPUと相性抜群)
- どのトークンからどのトークンへも**1ホップで直接**情報が届く——玉突きの伝言ゲームがないので、遠い記憶が薄れる問題も起きない

2017年、Googleの研究チームがこの問いに正面から答えた論文を発表します。タイトルは挑発的でした。

> **"Attention Is All You Need"(必要なのはAttentionだけ)**

この論文で提案されたアーキテクチャこそ、本書の主役——**Transformer(トランスフォーマー)**です。

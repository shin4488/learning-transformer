...
- [第5章 ニューラルネットワーク](05-neural-networks.md) — 層、行列計算としてのニューラルネットワーク
...
- 学習中に一度も見なかった「こねこみみ」が来ても、文字にばらしてから同じマージを順に適用すれば `[こ, ねこみみ]` と**既存の部品の組み合わせ**で必ず表せる。**未知語が発生しない**
...
> [!NOTE]
> 実際のトークナイザ(トークン化を行うプログラム)で日本語を処理すると、「猫」が1トークンになることもあれば、頻度の低い漢字が2〜3トークンに割れることもあります。本書では話を簡単にするため、通し例「猫は魚が好き」は常にきれいに `[猫, は, 魚, が, 好き]` の5トークンに分かれるものとして進めます。
...
**読み下し**: 8次元ベクトルのうち、猫のIDである5番目(0から数えて)の成分だけが1で、他は全部0。
...
**すべての単語ペアが等しく「無関係」**。one-hot表現は、「猫と犬は似ている」「魚と肉はどちらも食べ物」という、言葉の意味の構造をまったく表現できないのです。
...
![埋め込み空間のイメージ](images/ch06-embedding-space.png)

(図: 各点が1つの単語のベクトル。点の近さが意味の近さを表す。軸のラベルは人間による後付けの解釈)
...
![王 − 男 + 女 ≈ 女王のベクトル計算](images/ch06-king-queen.png)

(図: 「男 → 王」と「女 → 女王」の矢印は、どちらも同じ移動 (0, 2)。意味の関係が平行な矢印として現れている)

「王 − 男」は $(0, 2)$ 、つまり「男を王族にするための移動」です。同じ移動を「女」に適用すると「女王」に着く。**意味の関係(性別、王族らしさ)が、埋め込み空間では平行な矢印として現れている**わけです。他にも「東京 − 日本 + フランス ≈ パリ」(首都の関係)のような例が知られています。誰も教えていないのに、単語当てクイズの副産物にこんな構造が現れる。埋め込みという考え方の力がよく分かる例です。
...
数学や機械学習では、ベクトルは原則として「列ベクトル(縦向き)」として定義されます。そのため、行列 $E$ の左から掛けて $1 \times d_{\text{model}}$ のベクトルを取り出すには、転置記号 ${}^\top$ を付けて横向き(行ベクトル)にします。

$$
\mathbf{x}_{\text{猫}} = \mathbf{e}_{\text{猫}}^\top E
$$

**読み下し**: 猫のone-hotベクトル(縦向き)を転置して横ベクトルにしたものと、埋め込み行列 $E$ を掛けると、猫の埋め込みベクトルが出てくる($1 \times V_{\text{vocab}}$ と $V_{\text{vocab}} \times d_{\text{model}}$ の積なので、結果は $1 \times d_{\text{model}}$)。
...
- word2vecは「単語当てクイズ」の副産物として埋め込みを学習する(分布仮説)。「王 − 男 + 女 ≈ 女王」のように、意味の関係がベクトルの引き算・足し算に現れる

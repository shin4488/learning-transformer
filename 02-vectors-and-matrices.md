...
- **ベクトル**とは何か — 「数の並び」であり「矢印」でもある、という2つの見方
- ベクトルの足し算と**スカラー倍**
...
- 中身の1つ1つを **成分(component)** と呼び、添字で指します: $\mathbf{x} = (x_1,\ x_2)$ なら $x_1 = 170$ 、 $x_2 = 60$
- 本書では縦に並べる書き方(**縦ベクトル**)を基本にしますが、紙面の都合で $\mathbf{x} = (170, 60)$ と横に書くこともあります。中身は同じです
- ベクトルでないただの1個の数(3 とか −0.5 とか)は **スカラー(scalar)** と呼びます

3次元なら数が3個、100次元なら数が100個並びます。「100次元」と聞くと幾何学的に図示するのは困難ですが、その実体は「**数が100個並んだリスト**」に他なりません。
...
内積に進む前に、ベクトルの基本操作を2つだけ押さえます。どちらもとても簡単です。
...
**具体例1**: $\mathbf{a} = (3,\ 2)$ 、 $\mathbf{b} = (1,\ 4)$ のとき、
...
![内積の3パターン: 同じ向き・直角・逆向き](images/ch02-dot-product-patterns.png)

**パターン1: 向きが揃っている → 内積は大きい**

$\mathbf{a} = (3,\ 1)$ 、 $\mathbf{b} = (2,\ 1)$ 。どちらも「右上」を向いています。
...
$\mathbf{a} = (2,\ 0)$(真横)、 $\mathbf{c} = (0,\ 2)$(真上)。
...
![単語ベクトルの配置](images/ch02-word-vectors.png)
...
矢印には向きだけでなく長さがあります。ベクトルの長さを **ノルム(norm)** と呼び、 $\lVert \mathbf{a} \rVert$ と書きます。2次元なら、三平方の定理(直角三角形の斜辺の長さ)そのものです。
...
![ノルムは直角三角形の斜辺の長さ](images/ch02-norm.png)

(図: 右に3、上に4進むベクトルの長さは、底辺3・高さ4の直角三角形の斜辺 = 5)

ちなみに、**自分自身との内積**をとると $\mathbf{a} \cdot \mathbf{a} = a_1^2 + \cdots + a_n^2$ となり、ノルムの2乗に一致します。つまり $\lVert \mathbf{a} \rVert = \sqrt{\mathbf{a} \cdot \mathbf{a}}$ 。ノルムは内積から求められる、という関係になっています。
...
Transformerの内部(attention)では、シンプルで高速な**内積**がそのまま使われます(長さの影響が暴れる問題には、第8章で「 $\sqrt{d_k}$ で割る」という別の対処が登場します。これも、いま見た「長さの影響を抑える」工夫の一つです)。一方、「この2つの単語はどれくらい似ているか」を人間が調べるときは**コサイン類似度**がよく使われます。
...
行列がベクトルを「変換」するとはどういうことか、2次元の例で実際に見てみましょう。使う行列は次の $R$ です。
...
![90度回転行列の変換前後](images/ch02-rotation.png)

この $R$ は「**ベクトルを90°回転させる機械**」だったのです。行列によって、回転させるもの、引き伸ばすもの、押しつぶすもの、いろいろな変換機械が作れます。
...
この「行列 = 変換機械、行列積 = 変換の合成」という見方は、第5章で「ニューラルネットワークの層を重ねる」ことの意味を理解するときの土台になります(そこでは「行列を何個掛けても1個の行列にまとまってしまう=線形だけでは深くする意味がない」という重要な話につながります)。
...
つまり **$\mathbf{a}^\top \mathbf{b}$ という行列積の記法は、内積 $\mathbf{a} \cdot \mathbf{b}$ と同じもの**です。

さらに、この考え方は「たくさんのベクトルの内積を一度にまとめて計算する」ときに決定的な役割を果たします。

> [!TIP]
> **なぜ第8章の式で $QK^\top$ と転置するのか（超重要な先取り）**:
> 単語が $n$ 個あり、それぞれ $d$ 次元のベクトルだとします。これを行列にまとめると $n \times d$ 行列になります。
> 全単語同士の「すべての組み合わせの内積（類似度）」を計算したいとしましょう。
> もしそのまま $Q \times K$ と掛けようとしても、 $(n \times d) \times (n \times d)$ では「内側の次元（列数と行数）」が合わず、行列の掛け算ができません。
> 
> そこで、後ろの行列を転置して $K^\top$（$d \times n$）にします。すると、
> 
> $$
> (n \times d) \times (d \times n) \longrightarrow (n \times n)
> $$
> 
> 見事に掛け算ができるようになり、出力される $n \times n$ 行列の各マス $(i, j)$ に、「 $i$ 番目の単語と $j$ 番目の単語の内積」がすべて一撃で並びます。
> 論文や第8章の式で $QK^\top$ と転置が付いているのは、「内積を計算するために行列の向きを合わせる」という極めて合理的で必然的な理由によるものです。
...
| $\lVert \mathbf{a} \rVert$ | ノルム | ベクトルの長さ | $\lVert (3,4) \rVert = 5$ |

...
(図: 原点 o から + の点列をたどる線がベクトル $\mathbf{b} = (2, 1)$ 、 * の点列をたどる線が $\mathbf{a} = (3, 1)$ 。2本ともほぼ同じ「右上」を向いている → 内積は 7 で大きい正)
...
```text
  ...
  2 ||                     <- || の縦線が c = (0, 2)。真上向き
    ||
    ||
  0 o=====> x              <- = の横線が a = (2, 0)。真横向き
    0     2
```

(図: 原点 o から出る2本のベクトル。 $\mathbf{a}$ は真横、 $\mathbf{c}$ は真上で、なす角はちょうど直角 → 内積は 0)
...
(図: 軸の交点が原点。 * の点列をたどる線が $\mathbf{a} = (2, 1)$ で右上へ、 + の点列をたどる線が $\mathbf{d} = (-2, -1)$ で左下へ伸びる。2本は原点をはさんで一直線上の正反対 → 内積は −5 で負)
...
(図: 単語ベクトルの配置の例。猫・犬・魚は右上に集まり、銀行だけ左上に離れている)

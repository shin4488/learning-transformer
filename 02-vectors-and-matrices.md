...
- **「内積 = 類似度」という直感** — 第8章のAttentionを理解する鍵(本書全体の最重要ポイント)
...
矢印には「**向き**」と「**長さ**」があります。この幾何的なイメージこそが、このあと学ぶ「内積 = 類似度」の直感を支えます。
...
第8章のAttentionでは「各単語のベクトルに重み(0.7 とか 0.1 とかのスカラー)を掛けて足し合わせる」= **重み付き平均** という操作が心臓部になります。その部品は、いま学んだ「スカラー倍」と「足し算」だけです。
...
「配合率を変えてベクトルをブレンドする」——このイメージ、第8章まで持っていてください。

---

## 2.3 内積 — 本書で最も重要な演算

いよいよ本書全体の**最重要ポイント**に入ります。ここから先の数節は、本書のどのページよりも丁寧に読んでください。ここで得る直感が、第8章のAttention——Transformerの心臓部——を理解できるかどうかを決めます。
...
計算自体は小学生の算数です。**ベクトル2本を入れると、数が1個出てくる**。この「出てきた1個の数」に、驚くほど豊かな意味が宿っている——それをこれから見ていきます。
...
「計算ルールと幾何的意味がなぜ結びつくのか」を、腑に落ちる形で確認しておきましょう(厳密な証明ではなく、直感の確認です)。
...
以上を、本書全体を貫くスローガンとしてまとめます。額に入れて飾りたいくらい大事な一文です。
...
Transformerの心臓部 **Attention(注意機構)** は、「文中のある単語が、他のどの単語に注目すべきか」を決める仕組みです。そして、その「注目度」の計算方法が、まさに

> **ベクトルどうしの内積をとる**

ことなのです。第8章で学ぶ式 $QK^\top$ の実体は、「クエリというベクトルと、キーというベクトルの、**総当たりの内積**」にすぎません。つまり、

- 内積の計算ができて(2.3.1節)
- 内積 = 類似度という意味が分かっていれば(2.3.4節)

Attentionの核心はもう半分理解できたも同然です。**「第2章で学んだ内積が、第8章で効いてくる」——本書でいちばん大事な伏線**として、はっきり宣言しておきます。

```mermaid
graph LR
    A["内積の計算<br>掛けて足すだけ"] --> C["内積 = 類似度<br>という直感"]
...
名前の由来は、この値が2本の矢印のなす角度 $\theta$(シータ)の「コサイン $\cos\theta$」に一致するからです。三角関数を忘れていても大丈夫——「**−1〜+1に正規化された、向きだけの類似度**」と理解すれば十分です。
...
この見方が、本章の最後(2.8節)で「文章 = 単語ベクトルを積んだ行列」という形で花開きます。
...
```text
  ...
  1行目 (1,2) と (5,6) の内積:  1×5 + 2×6 = 17  ┐
                                                  ├→ ┌ 17 ┐
  2行目 (3,4) と (5,6) の内積:  3×5 + 4×6 = 39  ┘   └ 39 ┘
```
...
```text
          ← d_model(意味を表す次元)= 4 →
        ┌  1    0    2   -1  ┐   ← 1行目 = 「猫」のベクトル
   n    │  0    1    0    0  │   ← 2行目 = 「は」のベクトル
 トークン │  2    0    1   -1  │   ← 3行目 = 「魚」のベクトル
   数    │  0    1    0    1  │   ← 4行目 = 「が」のベクトル
  = 5   └  1    2    0    1  ┘   ← 5行目 = 「好き」のベクトル

          文 = ベクトルを縦に積んだ n × d 行列 X
```
...
(図: 文が行列になるまでの流れ。この行列 $X$ が本書後半の全登場人物の出発点になる)
...
実はこれこそが、Transformerが以前の技術(RNN、第7章)を打ち破った核心的な理由の1つです。RNNは単語を1個ずつ順番に処理するしかありませんでしたが、Transformerは文全体を1つの行列として同時に処理できるのです。この伏線は第7章・第8章で回収します。

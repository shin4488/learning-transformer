...
つまり、微分は「モデルの育て方」を、確率は「モデルの出力の読み方」を支える道具です。順番に見ていきましょう。
...
```mermaid
xychart-beta
    title "y = x^2 の曲線と、x = 1 での接線"
    x-axis "x" [0, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]
    y-axis "y" -1 --> 4
    line [0, 0.0625, 0.25, 0.5625, 1, 1.5625, 2.25, 3.0625, 4]
    line [-1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3]
```

(図: 下に膨らんだカーブが曲線 $y = x^2$ 、まっすぐな線が $x = 1$ での接線 $y = 2x - 1$ 。2本は接点 $(1, 1)$ で触れ合っています。この直線の傾き( $x$ が 1 増えると 2 上がる = 傾き 2)が、「 $x = 1$ における微分の値」です)
...
ちなみに第1章で紹介した $e^x$ は「微分しても $e^x$ のまま」という際立った性質を持ちます。「増える勢いが自分自身の値と等しい」と述べたのはこのことでした。
...
事実2を一行にまとめると「**傾きの逆方向に進め**」。実はこれこそが、機械学習の学習アルゴリズム(第4章の**勾配降下法**)の考え方のすべてです。あとはこれを「変数がたくさんある場合」に拡張するだけです。それが次の偏微分と勾配です。
...
![勾配と等高線](images/ch03-gradient-contour.png)

(図: 谷を真上から見た等高線図。緑の矢印 $\nabla f$ は最も急な上りの方向へ、オレンジの矢印 $-\nabla f$ は谷底へ向かう)
...
ここからは道具を切り替えて、**確率**の世界に入ります。言語モデルの出力を正しく理解するために欠かせない、確率の基本的な考え方と記法を整理します。
...
**具体例**: 明日の天気が「晴れ 0.6、くもり 0.3、雨 0.1」なら、 $0.6 + 0.3 + 0.1 = 1$ 。約束2を満たしています。もし合計が 0.9 や 1.2 になったら、それは確率として成り立っていません。

この「合計1」の約束は、第5章で学ぶ **softmax**(どんな数の並びでも「合計1の確率」に変換してくれる関数)の存在理由になります。

### 3.6.2 記法: $P(A)$

事象 $A$ の確率を $P(A)$ と書きます。読み方は「ピー・オブ・エー」または「 $A$ の確率」です。
...
**読み下し**: 「ピー・オブ・エー・ギブン・ビー」。「**$B$ が起こったという前提条件のもとで**、 $A$ が起こる確率」。縦棒 $\mid$ の右側が「前提条件」、左側が「知りたい事柄」

これを **条件付き確率(conditional probability)** と呼びます。

**身近な例**で感覚をつかみましょう。

- $P(\text{傘を持って出る}) = 0.2$ — 普段、傘を持って出かける確率は2割
- $P(\text{傘を持って出る} \mid \text{朝から雨}) = 0.95$ — **朝から雨が降っていると知っていれば**、傘を持つ確率は95%に跳ね上がる

**前提条件（情報）が増えると、確率は変化する。** これが条件付き確率の核心です。同じ「傘を持つ」という事柄でも、手元にある状況によって見積もりが 0.2 から 0.95 へと大きく変わります。

数で考える例も確認しておきます。52枚のトランプから1枚引くとき、

- $P(\text{ハートのエース}) = \frac{1}{52}$ — 何も情報がなければ全52枚のうちの1枚
- $P(\text{ハートのエース} \mid \text{引いたカードはハート}) = \frac{1}{13}$ — 「ハートだ」と分かった瞬間、候補はハート13枚に絞られる

```text
全体の可能性（52枚）  →  条件「ハート」（13枚に限定）  →  その中のエース（1/13）
```

**読み下し**: 条件が付くことで「分母となる世界の範囲」が52枚からハート13枚に狭まる。条件付き確率とは「前提条件によって分母を絞り込んだ上で測り直した確率」
...
![分布の形の3類型](images/ch03-distribution-shapes.png)
...
(図: 「猫は魚が___」の次の単語の確率分布。本来は語彙の全単語が横軸に並ぶが、図では上位の候補と「自動車」(ほぼ0)のみ表示。語彙の全単語まで含めれば、棒の高さの合計はちょうど1になる)
...
- **連鎖律**: 合成関数の変化率は各段の変化率の**掛け算**(歯車の比)。何十層のニューラルネットワークの学習(第5章の逆伝播)を可能にする仕掛け

...
これ以上の特別な前提知識は必要ありません。忘れていても無理なく読み進められるよう、必要な数学の道具はこの章で順番に復習します。
...
Transformerの解説書を開くと、最初に次のような数式を目にすることがよくあります。

$$
\mathrm{Attention}(Q, K, V) = \mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V
$$

**読み下し**: この式は第8章の主役です。現時点では「いくつかの記号が組み合わさっている」と眺めるだけで構いません

一見すると複雑に見えるこの式も、分解してみると使われている数学の部品は多くありません。**関数、行列、内積、指数関数、平方根、割り算**。基本となる部品はこれだけです。部品の意味と役割を1つずつ理解していけば、この式は自然と読めるようになります。

本書の第1〜3章は、そのための基礎となる道具を揃える章です。この第1章では、すべての土台となる「関数」と「数学の記号の読み方」を身につけます。

> [!TIP]
> **この章のポイント**: 数学の記号は、複雑な処理を短く正確に伝えるための「表記ルール」です。記号の読み方と「何を意味しているか」を押さえれば、数式は文章と同じように読み下せるようになります。まずは計算の細部にとらわれず、「何を入力して、何を出力しているか」に注目してください。
...
この「傾き」は、第3章で学ぶ**微分**の中心になる考え方です。頭の片隅に置いておいてください。
...
xychart-beta
    ...
    y-axis "y" 0 --> 9
    line [9, 6.25, 4, 2.25, 1, 0.25, 0, 0.25, 1, 2.25, 4, 6.25, 9]
```

(図: 二次関数 $f(x)=x^2$ のグラフ。左右対称の「谷」の形。 $x=0$ が谷底で、最小値 0 をとる)

注目してほしいのは、このグラフが「**谷**」の形をしていて、**谷底(いちばん低い点)がある**ことです。

なぜこれが大事なのか。実は機械学習の「学習」とは、「モデルの間違い度合い」を表す関数(損失関数と呼びます)の**谷底を探すこと**だからです。谷の形をした関数の底を、坂を転がり降りるようにして探す方法が、第4章で学ぶ**勾配降下法**です。二次関数の「谷」の絵には本書で何度も戻ってくるので、ここでしっかり覚えておいてください。
...
- $2^0 = 1$(0以外のどんな数でも、0乗は1と定義されています。なぜそう定義するのかは、すぐ下の表で説明します)
- $2^{-1} = \frac{1}{2}$ 、 $2^{-2} = \frac{1}{4}$(マイナス乗は「1をその数の累乗で割る」という意味です。 $2^{-2}$ なら、1を $2^2 = 4$ で割って $\frac{1}{4}$ )
...
xychart-beta
    title "指数関数 y = 2^x"
    x-axis "x" [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]
    y-axis "y" 0 --> 16
    line [0.25, 0.354, 0.5, 0.707, 1, 1.414, 2, 2.828, 4, 5.657, 8, 11.314, 16]
```
...
xychart-beta
    title "対数関数 y = log2(x)"
    x-axis "x" [0.5, 1, 2, 3, 4, 5, 6, 7, 8]
    y-axis "y" -1 --> 3
    line [-1, 0, 1, 1.585, 2, 2.322, 2.585, 2.807, 3]
```

(図: 対数関数 $\log_2 x$ のグラフ。増えるには増えるが、どんどん増え方がゆるやかになる。 $x=1$ でちょうど 0 を通る)

指数関数が「爆発」なら、対数はその爆発を**扱いやすい大きさに戻すもの**です。1024 という大きな数も、対数を通すと 10 というおとなしい数になります。100万を超える $2^{20}$ も、対数を通せばただの 20 です。
...
**読み下し**: 「掛け算の対数」は「それぞれの対数の足し算」に分解できる

なぜこれが成り立つのか、具体例で確認してみましょう。

```text
元の数（掛け算の世界）:      4     ×     8     =    32
                           ↑           ↑          ↑
指数の形（累乗の世界）:   (2の2乗)  ×  (2の3乗)  = (2の5乗)  ※ 2+3=5
                           ↓           ↓          ↓
対数の値（何乗かの世界）:  log2(4)=2  +  log2(8)=3  = log2(32)=5
```

元の数で「 $4 \times 8 = 32$ 」と掛け算をしている操作は、指数の世界では「 $2 + 3 = 5$ 」という足し算に対応しています。対数 $\log_2$ は「指数の肩に乗っている数字（何乗か）」を取り出す道具なので、**掛け算をきれいさっぱり足し算に変換できる**のです。

機械学習では、「確率を何十回も掛け合わせる」計算が頻出します（例:「文全体の確率 = 各単語の確率の掛け算」— 第7章で登場します）。掛け算が何個も連なると手計算もコンピュータの計算も扱いづらくなりますが、対数を取ればすべて足し算（$\Sigma$）に置き換わり、計算も理論的な分析も格段に扱いやすくなります。

**理由2: 極端に小さい数・大きい数を安全に扱える**

確率は 0 から 1 の間の数です。たとえば $0.1$ を100回掛け合わせると、 $0.1^{100} = 10^{-100}$ となり、小数点以下に0が99個も並ぶ極めて微小な数値になります。コンピュータでこのような小さすぎる小数を掛け続けると、表現できる桁数の限界を下回って値がゼロとみなされてしまい（アンダーフロー）、計算が破綻してしまいます。

そこで対数を取ると、 $10^{-100}$ は $\log_{10}(10^{-100}) = -100$ という、扱いやすい整数（負の数）に変換されます。

| 元の数 | 対数($\log_{10}$)をとった値 | 特徴 |
|---|---|---|
| 1,000,000 | 6 | 桁数がそのまま数値になる |
| 1,000 | 3 | |
| 1 | 0 | 1の対数は常に0 |
| 0.001 | −3 | 小数点以下のゼロの個数が負の数で並ぶ |
| 0.000001 | −6 | |

(表: 桁違いに離れた数値が、対数を取ると「6, 3, 0, −3, −6」と等間隔に並ぶ。対数は「桁数を測るものさし」として機能する)

この性質は、第5章の**交差エントロピー**(モデルの採点法)や、第13章の**スケーリング則**(両対数グラフ)で直接活用されます。

### 1.5.3 自然対数 $e$ と $\ln$

対数の底には 2 や 10 のほか、数学や機械学習で標準的に使われる特別な数 $e$(**ネイピア数**、約 2.718)があります。底を $e$ とする対数を **自然対数(natural logarithm)** と呼び、 $\ln x$ または底を省略して単に $\log x$ と表記します(機械学習の論文や教科書で底が書かれていない $\log$ は、原則として自然対数です)。

なぜ $2$ や $10$ のようなキリの良い数字ではなく、約 $2.718$ という半端な数をわざわざ使うのでしょうか。

> [!NOTE]
> **なぜ底に $e$ を選ぶのか**:
> 最も大きな理由は、**微積分の計算が圧倒的にきれいになるから**です。第3章で学ぶ「微分（傾きの計算）」において、関数 $f(x) = e^x$ は「微分してもまったく形が変わらず $e^x$ のまま」という唯一無二の性質を持ちます。底が 2 や 10 だと微分するたびに余計な係数が掛かって式が複雑になりますが、底が $e$ であれば余分な係数が一切出ず、ニューラルネットワークの学習計算が最もシンプルになります。

本書においては、「 $e \approx 2.718$ という定数であり、 $e^x$ は $2^x$ と同様に右肩上がりに急増する指数関数、 $\ln$ はその逆の対数」と押さえておけば十分です。第5章の **softmax** では $e^x$ が、**交差エントロピー** では $\ln$ が計算の中心になります。
...
と書けます($w$ は word の頭文字)。「文とは単語の列 $w_1, w_2, \dots, w_n$ である」という表記は、本書全体で使い続けます。

### 1.8.2 添字が2つ付くこともある

第2章で学ぶ**行列**では、 $x_{ij}$ のように添字が2つ付きます。「 $i$ 行目・ $j$ 列目の値」という意味で、座席表で「前から $i$ 番目・左から $j$ 番目」と席を指すようなものです。今は「添字は背番号。2つ付いたら縦横の座席番号」とだけ覚えておいてください。
...
この2つを**直列につなぐ**とどうなるでしょうか。まず $g$ に入れて、その出力をそのまま $f$ に入れるのです。図にするとこうなります(この「機械の直列つなぎ」はこの章の最重要図です)。
...
$x = 5$ なら $g(f(5)) = 10 + 3 = 13$ 。さきほどの 16 と違います。**合成は順番が大事**です。料理で「切ってから焼く」と「焼いてから切る」が別物なのと同じです。この事実は、第2章の行列の掛け算(こちらも順番で結果が変わります)への伏線になっています。
...
96台も直列につないだ機械を「学習」させるには、「最後の出力の誤差に対して、各機械がどれだけ責任を負っているか」を計算する必要があります。それを可能にするのが、合成関数の微分ルール=**連鎖律(chain rule)** です。第3章で登場し、第5章の**逆伝播**で本領を発揮します。

```mermaid
graph TD
    A["第1章<br>関数の合成"] --> B["第3章<br>連鎖律<br>(合成関数の微分)"]
    A --> C["第5章<br>ニューラルネットワーク<br>= 層の積み重ね"]
    B --> D["第5章<br>逆伝播<br>(学習の中核)"]
    C --> D
```
...
| $\arg\max$ | アーグマックス | 最大値を出した場所・番号 | $\arg\max(3,8,5)=2$ 番目 |
...
- **指数関数** $2^x$ は爆発的に増える。**対数** $\log$ はその逆で、「何乗?」に答え、掛け算を足し算に変え、桁違いの数を扱いやすくする。だから機械学習は対数だらけ
...
次の章では、**ベクトルと行列**を学びます。Transformerの中では、単語も文も「数の並び(ベクトル)」として扱われます。そして本書全体で一番大事な考え方である「**内積は類似度である**」が、いよいよ登場します。この考え方が、第8章のattentionを理解する鍵になります。

...
第9章で学んだTransformerの原論文 "Attention Is All You Need"(2017年)は、**翻訳のための機械**でした。入力文を読み込むエンコーダと、訳文を生み出すデコーダがcross-attentionでつながる、二階建ての構造です。
...
**GPT(Generative Pre-trained Transformer)** は、原論文の**デコーダだけ**を取り出したモデルです。ただしエンコーダがいないので、エンコーダを参照するためのcross-attention(第9章)は不要になり、取り除かれます。残るのは、**「因果マスク付きself-attention + FFN(+残差接続とLayerNorm)のブロックを $N$ 層積んだもの」** という、第9章のデコーダよりさらに素直な構造です。重要なのは**因果マスクが残っている**こと。各トークンは自分自身と、それより左(過去)しか見られません。

構造を1枚の図にしておきましょう。

```mermaid
flowchart TD
    IN["入力: 猫 は 魚 が 好き"] --> EMB["埋め込み + 位置エンコーディング(第6・9章)"]
    EMB --> SA
    subgraph BLK["デコーダブロック × N(同じブロックを積むだけ。cross-attention はない)"]
        SA["因果マスク付き self-attention(+残差・LayerNorm)"] --> FFN["FFN(+残差・LayerNorm)"]
    end
    FFN --> LM["線形層 + softmax(第9章)"]
    LM --> OUT["出力: 各位置の「次の単語」の確率分布"]
```

エンコーダも、エンコーダにつなぐcross-attentionもない、「一種類のブロックの積み重ね」。この徹底した単純さが、後で見る「スケールのしやすさ」に効いてきます。

### 11.2.2 訓練: 第10章の次単語予測そのもの

GPTの訓練問題は **次単語予測** です。これは**第10章で学んだ自己教師あり学習そのもの**です。「猫は魚が」から「好き」を当てる、全位置同時に採点する、損失は交差エントロピーの平均という仕組みがすべてそのまま当てはまります。
...
第10章で学んだ道具(各位置の交差エントロピーの平均損失、パープレキシティによる評価)がそのまま使えます。第10章で追った訓練プロセスは、まさにGPT型モデルの学習そのものでした。
...
GPTの因果マスクには、訓練の都合上やむを得ない「もったいなさ」があります。「猫は魚が好き」の「魚」の意味を考えるとき、GPT型では左側の「猫は」までしか見られず、後ろに続く「が好き」は見えません。でも人間が文を**読解**するときは、前後の両方を見ますよね。「魚」の後ろに「が好き」が続くと知っていれば、この「魚」は食べ物(えさ)としての魚だと分かります。
...
なお、実際のBERTの訓練には細かい工夫があります。選んだ15%のトークンをすべて `[MASK]` に置き換えるのではなく、そのうち8割だけを `[MASK]` にし、1割はランダムな別の単語に、1割は元の単語のままにして「本当は何だったか」を当てさせます。本番でBERTを使うとき入力に `[MASK]` は現れないため、「マスクがなくても全トークンについて良い表現を作る」練習を混ぜておく、という配慮です(細部まで覚える必要はありません)。また、初代BERTは穴埋めと並行して「2つの文はもともと連続していたか?」を当てる問題(次文予測)も解いていましたが、後の研究であまり効果がないと分かり、使われなくなりました。
...
```mermaid
flowchart TD
    IN["入力: [CLS] 猫 は [MASK] が 好き"] --> EMB["埋め込み + 位置エンコーディング"]
    EMB --> ENC["エンコーダブロック × N<br>マスクなし self-attention + FFN<br>(全トークンが互いに見える)"]
    ENC --> U1["各 [MASK] 位置のベクトル<br>→ 線形層 + softmax → 「元の単語」の分布"]
    ENC --> U2["[CLS] 位置のベクトル<br>→ 分類器を載せて「文全体の要約」として利用"]
```
...
名前の由来にも触れておきます。T5という略称は、Text-to-Text Transfer Transformer に5つ含まれる頭文字のTから来ています。このうちの **Transfer(転移)** は、「事前学習で得た力を、別のタスクへ移して使う」という **転移学習(transfer learning)** の考え方を指しています。実は「大量データで事前学習し、目的のタスクで仕上げる」という本章の3系譜すべてに共通する流れが、転移学習の一種です。
...
| 年 | 出来事 |
|---|---|
| 2017年 | Transformer原論文 "Attention Is All You Need"(翻訳用) |
| 2018年6月 | GPT-1(デコーダのみ・約1.2億パラメータ) |
| 2018年10月 | BERT(エンコーダのみ・最大約3.4億)。理解タスクの記録を総なめに |
| 2019年2月 | GPT-2(約15億)。流暢な生成で話題に |
| 2019年10月 | T5(最大約110億)。「テキスト→テキスト」への統一 |
| 2020年 | GPT-3(約1,750億)。few-shotの発見 |
| 2022年 | ChatGPT。デコーダのみ+対話の仕上げ(第12章) |
| 現在 | 主要LLMはほぼすべて「デコーダのみ」の子孫 |
...
| 4 | 「続きを書く」はfew-shotや対話AIへの追加訓練に直結する | 事前学習の能力をそのまま対話に使える |

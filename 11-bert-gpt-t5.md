...
**GPT(Generative Pre-trained Transformer)** は、原論文の**デコーダだけ**を取り出したモデルです。ただしエンコーダがいないので、エンコーダを参照するためのcross-attention(第9章)は不要になり、取り除かれます。残るのは、

> **因果マスク付きself-attention + FFN**(+残差接続とLayerNorm)のブロックを $N$ 層積んだもの

という、第9章のデコーダよりさらに素直な構造です。重要なのは**因果マスクが残っている**こと。各トークンは自分より左(過去)しか見られません。
...
左下半分だけが○の三角形——GPTの世界では、時間は常に左から右へしか流れません。
...
GPT-1からGPT-3まで、**構造はほぼ同じで、パラメータ数が約1,500倍**になっています。「大きくしただけ」で何が起きたのか——few-shotの驚きは第12章で、規模と賢さの法則は第13章で詳しく扱います。
...
```text
   各[MASK]位置の「元の単語」の分布    [CLS]位置の要約ベクトル
                ↑                          ↑
         線形層 + softmax            (分類器を載せて利用)
                ↑                          ↑
    ┌─────────────────────────────────────────────┐
    │  エンコーダブロック × N                        │
    │  マスクなしself-attention + FFN               │
    │  (全トークンが互いに見える)                    │
    └─────────────────────────────────────────────┘
                        ↑
          埋め込み + 位置エンコーディング
...
**T5(Text-to-Text Transfer Transformer)** は、これを一刀両断にしました。
...
ここまでの内容を一望します。まず本章の最重要図である系統図を、MermaidとASCIIの両方で示します。
...
graph TD
    ...
    style G fill:#ffd54f,stroke:#f57f17,stroke-width:3px
    style L fill:#ffe082,stroke:#f57f17
```

```text
                 Transformer 原論文(2017)
                エンコーダ + デコーダ(翻訳用)
                           │
      ┌────────────────────┼────────────────────┐
      │                    │                    │
 エンコーダだけ         デコーダだけ           両方そのまま
      │                    │                    │
      v                    v                    v
 ┌──────────┐      ┌──────────┐      ┌──────────┐
...
> **BERTは読む人、GPTは書く人、T5は訳す人。**

年表の形でも整理しておきましょう。

```text
2017 ──● Transformer原論文「Attention Is All You Need」(翻訳用)
        │
2018 ──●── 6月  GPT-1(デコーダのみ・約1.2億)
        │
        ●── 10月 BERT(エンコーダのみ・最大約3.4億)
        │        → 理解タスクの記録を総なめに
2019 ──●── 2月  GPT-2(約15億)— 流暢な生成で話題に
        │
        ●── 10月 T5(最大約110億)— テキスト→テキスト統一
        │
2020 ──●── GPT-3(約1,750億)— few-shotの発見
        │
2022 ──●── ChatGPT — デコーダのみ+対話の仕上げ(第12章)
        │
...
という非対称があります。T5の「すべてはテキスト→テキスト」という洞察は正しかったのですが、皮肉なことに、デコーダのみでもプロンプト(入力文)の中に材料を全部入れてしまえば同じことができると分かってきました。**1つのモデルで全部済むなら、生成できる方を選ぶ**のが自然です。
...
という利点になります。「とにかく大きくすると賢くなる」というスケーリング則の時代(第13章)には、**大きくしやすい構造そのものが正義**でした。
...
### 理由4: 「事前学習→そのまま対話」への道がまっすぐ

GPT型の訓練(続きを書く)は、そのまま「プロンプトの続きとして答えを書く」という使い方に直結します。例を見せれば真似をするfew-shot(第12章)も、指示に従う対話AIへの仕立て直し(同じく第12章)も、「続きを書く機械」という土台の上に素直に築けました。BERT型にはこの道がありません。
...
| 4 | 「続きを書く」はfew-shot・SFT・対話AIへ直結する | 対話AIへの道がまっすぐ |
...
この一行は、本書のここまでの旅の集大成です。分解してみましょう。
...
**Q: では、T5の路線は「負けた」の?**
A: 負けたというより、**思想が吸収された**と言うべきでしょう。「すべてのタスクをテキストで統一する」というT5の洞察は、プロンプトひとつで何でも指示する現代LLMの使い方そのものに受け継がれています。FLAN-T5のように現役で使われるモデルも残っています。
